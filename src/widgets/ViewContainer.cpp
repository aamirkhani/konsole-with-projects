    ...
    , _projectManager(new ProjectManager(this))
{
    ...
    _projectManager->createProject(QStringLiteral("Main"));
    
    // Connect to project changes to filter visible tabs
    connect(_projectManager, &ProjectManager::activeProjectChanged, this, &TabbedViewContainer::onActiveProjectChanged);

    auto tabBarWidget = new DetachableTabBar();
    setTabBar(tabBarWidget);
    setDocumentMode(true);
    setMovable(true);
    setTabPosition(QTabWidget::North);  // Show tabs at the top
...
void TabbedViewContainer::addView(TerminalDisplay *view)
{
    ...
    // Assign terminal to active project and format title
    if (_projectManager) {
        Project *activeProject = _projectManager->activeProject();
        if (activeProject) {
            activeProject->addTerminal(view);
            _terminalToProject[view] = activeProject;
            // Format title with project name
            QString projectName = activeProject->name();
            QString tabTitle = QStringLiteral("[%1] %2").arg(projectName, item->title());
            index = insertTab(index, viewSplitter, item->icon(), tabTitle);
        } else {
...
void TabbedViewContainer::activateNextView()
{
    ...
    // Find next visible tab after current
    ...
    // No visible tab found after current, go to next project's first tab
    activateNextProject();
}
...
void TabbedViewContainer::activatePreviousView()
{
    ...
    // Find previous visible tab before current
    ...
    // No visible tab found before current, go to previous project's last tab
...
ProjectManager *TabbedViewContainer::projectManager()
{
    return _projectManager;
}

Project *TabbedViewContainer::currentProject() const
{
    if (!_projectManager) {
        return nullptr;
    }
    return _projectManager->activeProject();
}

void TabbedViewContainer::assignTerminalToProject(TerminalDisplay *terminal, Project *project)
{
    if (!terminal || !project) {
        return;
    }
    ...
    if (_terminalToProject.contains(terminal)) {
        Project *prevProject = _terminalToProject[terminal];
        if (prevProject) {
            prevProject->removeTerminal(terminal);
        }
    }
    
    // Add to new project
    project->addTerminal(terminal);
    _terminalToProject[terminal] = project;
    
    // Update tab title with project name
    int idx = indexOf(qobject_cast<QWidget*>(terminal->parent()->parent()));
    if (idx >= 0) {
        QString tabTitle = QStringLiteral("[%1] %2").arg(project->name(), tabText(idx));
        // Remove old project prefix if any
        if (tabTitle.contains(QStringLiteral("] "))) {
            int pos = tabTitle.indexOf(QStringLiteral("] "));
            if (pos > 0) {
                tabTitle = QStringLiteral("[%1] %2").arg(project->name(), tabTitle.mid(pos + 2));
            }
        }
        setTabText(idx, tabTitle);
    }
}

void TabbedViewContainer::onActiveProjectChanged(Project *project)
{
    if (!project) {
        return;
    }
    
    const QList<QPointer<TerminalDisplay>> &projectTerminals = project->terminals();
    
    // Show/hide tabs based on whether they belong to the active project
    for (int i = 0; i < count(); ++i) {
        auto splitter = qobject_cast<ViewSplitter*>(widget(i));
        if (!splitter) {
            continue;
        }
        
        // Get the active terminal from this splitter
        TerminalDisplay *terminal = splitter->activeTerminalDisplay();
        
        bool inActiveProject = terminal && projectTerminals.contains(terminal);
        
        // Use setTabVisible if available (Qt 5.15+)
        tabBar()->setTabVisible(i, inActiveProject);
    }
    
    // Switch to first visible tab if current is not visible
    int currentIdx = currentIndex();
    if (!tabBar()->isTabVisible(currentIdx)) {
        for (int i = 0; i < count(); ++i) {
            if (tabBar()->isTabVisible(i)) {
                setCurrentIndex(i);
                break;
...
void TabbedViewContainer::activateNextProject()
{
    if (!_projectManager) {
        return;
    }
    
    Project *currentProj = _projectManager->activeProject();
    int currentIdx = _projectManager->projects().indexOf(currentProj);
    
    // Go to next project
    int nextIdx = (currentIdx + 1) % _projectManager->projectCount();
    Project *nextProject = _projectManager->projectAt(nextIdx);
    
    if (nextProject) {
        _projectManager->setActiveProject(nextProject);
        // Switch to first visible tab in next project
        for (int i = 0; i < count(); ++i) {
            if (tabBar()->isTabVisible(i)) {
                setCurrentIndex(i);
                break;
...
void TabbedViewContainer::activatePreviousProject()
{
    if (!_projectManager) {
        return;
    }
    ...
    int prevIdx = (currentIdx - 1 + _projectManager->projectCount()) % _projectManager->projectCount();
    Project *prevProject = _projectManager->projectAt(prevIdx);
    
    if (prevProject) {
        _projectManager->setActiveProject(prevProject);
        // Switch to last visible tab in previous project
        for (int i = count() - 1; i >= 0; --i) {
            if (tabBar()->isTabVisible(i)) {
                setCurrentIndex(i);
                break;
...
void TabbedViewContainer::activateProjectByIndex(int index)
{
    if (!_projectManager || index < 0 || index >= _projectManager->projectCount()) {
        return;
    }
    
    Project *project = _projectManager->projectAt(index);
    if (project) {
        _projectManager->setActiveProject(project);
        // Switch to first visible tab in that project
        for (int i = 0; i < count(); ++i) {
            if (tabBar()->isTabVisible(i)) {
                setCurrentIndex(i);
                break;
