...
#include "widgets/ProjectTabBar.h"
#include "Project.h"
...
void ViewManager::setupActions()
{
    ...
    // Add Ctrl+1, Ctrl+2, etc. for switching projects (up to 9 projects)
    ProjectManager *projectManager = _viewContainer->projectManager();
    if (projectManager) {
        for (int i = 0; i < 9; ++i) {
            action = new QAction(i18nc("@action Switch to project N", "Switch to Project %1", i + 1), this);
            collection->addAction(QStringLiteral("switch-to-project-%1").arg(i), action);
            collection->setDefaultShortcut(action, QKeySequence(Qt::CTRL | static_cast<Qt::Key>(Qt::Key_1 + i)));
            connect(action, &QAction::triggered, this, [this, i]() {
                _viewContainer->activateProjectByIndex(i);
            });
            _viewContainer->addAction(action);
        }
    }
...
TabbedViewContainer *ViewManager::createContainer()
{
    ...
    // Create wrapper widget with project tab bar at top and terminal tab container below
    auto *wrapper = new QWidget();
    auto *layout = new QVBoxLayout(wrapper);
    layout->setContentsMargins(0, 0, 0, 0);
    layout->setSpacing(0);
    
    // Add project tab bar at the top
    ProjectManager *projectManager = container->projectManager();
    if (projectManager) {
        auto *projectTabBar = new ProjectTabBar(projectManager, wrapper);
        // When "+" is clicked, create new project AND auto-create first terminal
        connect(projectTabBar, &ProjectTabBar::newProjectRequested, this, [this, projectManager, container]() {
            Project *newProject = projectManager->createProject();
            if (newProject) {
                // Auto-create first terminal in new project
                projectManager->setActiveProject(newProject);
                Q_EMIT newViewRequest();
            }
        });
        layout->addWidget(projectTabBar);
    }
    
    // Add the terminal tab container below
    layout->addWidget(container, 1);
    
    _containerWrapper = wrapper;
